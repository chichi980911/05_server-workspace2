...
<body>
	...
	<h1>1.변수(속성 == attribute)</h1>
	<pre>
		*변수 선언과 동시에 초기화 (c:set var="" value="" [scope=""])
	
		-변수 선언하고 초기값을 대입해주는 기능을 제공
		-해당 변수를 어떤 scope에 담아둘껀지 지정가능함(생략시 기본적으로 pageScope에 담김)
		=> 즉, 해당 scope에 setAttribute 를 통해서 key=value 형태로 데이터로 담아ㅈ놓는 거라고 생각하며
		=>c:set 으로 선언된 변수는 SEL로 접근해서 사용가능 (스크립팅 원소로는 접근 불가)
		-변수 타입을 별도로 지정하지 않음
		-초기값을 반드시 지정해줘야 함
	</pre>
	...
	<c:set var="num2" value="20" scope="request" />  <!-- pageContext.setattribute("num1",20 ) -->
	
	
	
	<c:set var="result" value="${num1+num2}" scope="session"/>

	result q변수값 : ${result }<br><br>
	...
	*변수 삭제 (c:remove var = "제거하고자 하는 변수명" [scope=""])
	...
	scope.removeAttribute를 통해 제거하는것
	
	</pre>	
	
	삭제된 result : ${result}<br><br>
	...
	<%} %>
	--%>
	<c:if test="${str eq '안녕하세요'}">나다</c:if>
	
	<c:if test='${str ne "안녕하세요"}'>내가 아니다</c:if>
	
	 
	<h3>3. 조건문 - choose(c:choose, c:when, c:otherwise)</h3>
	<pre>
		-JAVA의 if-else, if-else if 문과 비슷한 역할을 하는 태그
		-각 조건들을 c:choose의 하위요소로  c:when을 통해서 작성(else == outherwise)
	</pre>
	<%-- 
	<%if(num1 > 20) { %>
	...
	-향상된 for문 - (c:forEach var="변수명" items="순차적으로 접근하고자하는 객체(배열|컬렉션)" [varStatus="현재접근된 요소의 상태값을 보관할 변수명"])
	
	var 속성으로 선언된 변수에 접근하고자 할 떄 는 반드시 EL구문으로 접근해야한다.
		...
		<c:choose>
			...
			<c:otherwise>
				<c:forEach var="p" items="${plist}" varStatus="status">
					<tr>
						<td>${status.count}</td> <!-- index 속성: 0부터시작 /count속성 :1부터 시작 -->
						<td>${p.name}</td>
						<td>${p.age}</td>
						<td>${p.gender}</td>
					</tr>
	...
	<pre>
	(c:forTokens var="변수명" items="분리시키고자 하는 문자열")
	...
	<ol>
		<c:forTokens var="d" items="${device}" delims=",./"></c:forTokens>
		<li>${d }</li>
	</ol>
